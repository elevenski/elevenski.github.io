window.addEventListener('DOMContentLoaded', async function () {
    ...
    document.querySelectorAll('.github-card').forEach(async function (el) {
      ...
      <div data-id="${data.full_name || 'NOT_FOUND'}" onclick="window.open('https://github.com/${data.full_name || elevenvac}')" class="oal-comps-xl-card rounded-cxl mt-3">
          <p class="section-regular-text-b oal-comps-xl-card-link">elevenvac/</a>${data.name || "RG EMİR"}</p>
          ...
          <p class="section-regular-text opacity-80">${data.description || "Description not found."}</p>
      </div>
        `;
    });
  });
