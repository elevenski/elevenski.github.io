window.addEventListener('DOMContentLoaded', async function() {
    ...
    filtered.slice(0, 6).forEach(repo => {
        ...
        card.innerHTML = `
<div data-id="${repo.full_name}" onclick="window.open('${repo.html_url}?utm_source=eleven.js.org')" class="cardNH rounded-3xl h-full cursor-pointer">
    <div class="p-4">
        <div class="flex items-center justify-between">
            <p class="textNormal textBold opacity-80 truncate">${repo.name}</p>
            <p class="textThin opacity-60 text-sm"><i class="fa-light fa-star mr-1"></i> ${repo.stargazers_count}</p>
        </div>
        <p class="textThin opacity-60 mt-2 truncate-2">
